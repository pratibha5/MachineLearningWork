...
public class ClassificationResult {
	...
	public ClassificationResult(RuleModel mod, Dataset train, Dataset test) {
		this.train = train;
		this.test = test;
		//bAccuracy = 0;
	...
	private void calcSensSpecBacc() {
		...
		//// Bug! If sens is calculated for each class, every class gets the average 
		//// sens! --PG20090811 
		//// For each class, compute the statistics for the class
	...
	public static String confusionMatrixString(double[][] cm, VHierarchyNode vhn) {
		int maxW = (int) Math.floor(Math.log10(util.MathUtil.max(cm))) + 1;
	...
	/* A simpler confusionMatrixString() implementation 
	public static String confusionMatrixString(double[][] cm, VHierarchyNode vhn) {
		StringBuffer buf = new StringBuffer();
		buf.append("Confusion matrix:\n");
		for (int i = 0; i < cm.length; i++)
			buf.append("\tc" + (i + 1) );
		buf.append("\tAbstentions");
		for (int i = 0; i < cm.length; i++) {
			buf.append("\n");
			for (int j = 0; j < cm[i].length; j++)
				buf.append("\t" + (int) cm[i][j]);
			buf.append("\t|\tc" + (i + 1) + "  =  " + vhn.getValue(i).getName());
		}
		return buf.toString(); 
	}
