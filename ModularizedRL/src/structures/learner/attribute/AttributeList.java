...
public class AttributeList extends ArrayList {
	 ...
	 */
	public void defineAttributes(Dataset data) throws Exception {
	...
	public String toString(ArrayList predictions, Dataset trainData) {
	 ...
	 * 'index'.
	 */
	public void computeInfoGains(Dataset train) {
		for (int i = 0; i < size(); i++) {
	 ...
	 */
	protected LearnerAttributeType guessAttributeType(Dataset dl, int index) {
	 ...
	 */
	public void computeInfoGain(int attributeIndex, Dataset train) {
		...
		for (int x = 0; x < p.length; x++) {
			...
			entropy += (-1) * p[x] * util.MathUtil.log2(p[x]);
		}
	 ...
	 */
	protected double entropy(Object value, int attributeIndex, Dataset train) {
		...
		for (int x = 0; x < p.length; x++) {
			p[x] = (nSv == 0) ? 0 : (p[x] / nSv);
			dReturn += (-1) * p[x] * util.MathUtil.log2(p[x]);
		}
