...
public class Util {
	...
	private static Instance createInstance(Dataset trainData, Dataset origData,
	 ...
	 */
	public static Instances[] createInstances(Dataset train, Dataset test, boolean uD) 
				throws AttributeDoesNotExistException, InstanceNotFoundException {
	 ...
	 * @throws InstanceNotFoundException
	 */
	public static Instances createInstances(Dataset train, boolean uD)
			throws AttributeDoesNotExistException, InstanceNotFoundException {
		Instances[] inst = createInstances(train, null, uD);
		return inst[0];
	}

	public static void discDatasets(Dataset trn, Dataset tst, int meth,
			double methVal) throws AttributeDoesNotExistException {
		Dataset[] sets = new Dataset[2];
		sets[0] = trn;
		sets[1] = tst;
		discDatasets(sets, meth, methVal);
	}

	public static void discDatasets(Dataset[] dats, int meth, double methVal) throws AttributeDoesNotExistException {
	...
	public static void discDatasetsWithoutTrim(Dataset[] dats, int meth,
			double methVal) throws AttributeDoesNotExistException {
		Discretizers d = new Discretizers();
		d.setMehtod(meth, methVal);
		d.setData(dats[0]);
	...
	public static void discDataset(Dataset dats, int meth, double methVal) {
		Discretizers d = new Discretizers();
		d.setMehtod(meth, methVal);
		d.setData(dats);
		double[][] pts = d.discretizeMDL();
	...
	public static void discDatasetWithoutTrim(Dataset dats, int meth,
			double methVal) {
		Discretizers d = new Discretizers();
		d.setMehtod(meth, methVal);
		d.setData(dats);
	...
	public static Instances[] createInstances(Dataset train, Dataset test,
