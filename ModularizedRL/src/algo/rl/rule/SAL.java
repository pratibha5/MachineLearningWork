...
public class SAL implements RuleGenerator {
	...
	public RuleList generateRules() {
		...
		if (parameters.verbosity > 2) {
			...
			for (int i = 0; i < attList.size(); i++) {
				System.out.print(((structures.learner.attribute.LearnerAttribute) attList.get(i)).getName() + " ");
			}
			System.out.println();
		}
	...
	private RuleList getModelAndEnd(PriorityQueue inBeam) {
		...
		if (parameters.getPriorRules() != null && (!parameters.shouldDoBss() || parameters.verbosity > 1))
			System.out.println("Retained " + nRetainedRules 
					+ " rules, " + transferredAtts.size() + " atts"
					+ " in the new model of " + model.size() + " rules, "
					+ model.getAttributes().size() + " atts.");
