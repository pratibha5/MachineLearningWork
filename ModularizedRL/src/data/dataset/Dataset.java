...
public class Dataset {
	...
	public Dataset(ArrayList<Attribute> atts, String fileName, String seperator) {
	...
	public void setInstances(double[][] insts) {
		...
		//System.out.println("Dataset.setInstances() " + instCovered.length);
	}
	 ...
	 */
	public data.dataset.Instance getInstance(int inst)
			throws ValueNotFoundException {
		...
		return new data.dataset.Instance(
	 ...
	 * @throws AttributeDoesNotExistException 
	 */
	public void setDiscretization(double[][] pts, Dataset ref) throws AttributeDoesNotExistException {
	...
	*/
	...
	public Dataset getSubset(int[] insts) throws NullDatasetException {
		if (insts.length == 0)
			throw new NullDatasetException();
		Dataset tmpD = new Dataset(cloneAttributes(), fileName, sep);
	...
	public Dataset testCV(int folds, int f)
	...
	public Dataset trainCV(int folds, int f)
			throws NullDatasetException {
		if (foldSplits == null)
			generateNewFolds(folds);
		if (foldSplits.length != folds)
	 ...
	 */
	public void setNewDoubleValues(double[][] attVals, Dataset trainData) {
	...
	public Dataset trainSplit(double percent) {
		if (foldSplits == null)
	...
	public Dataset testSplit(double percent) {
		if (foldSplits == null)
			generateNewFolds((int) (100 / percent));
		if (foldSplits.length != (int) (100 / percent))
			generateNewFolds((int) (100 / percent));
	...
	public Dataset rrvTrainSplit(int percent) {
	...
	public Dataset rrvTestSplit(int percent) {
		int[] insts = generateRandomRRV(percent);
		Dataset tmpD;
		try {
			tmpD = getSubset(insts);
	...
	public void equalizeFromTestDataset(Dataset tstD) {
	...
	public void equalizeFromTrainDataset(Dataset train) {
	 ...
	 */
	public Dataset getSubset(String[] instanceNames, boolean reverse) {
	...
	public Instance wekaInstance(int ts, FastVector wekaAtts, boolean uD) {
		...
		data.dataset.Attribute classAtt = classAttribute();
		try {
	...
	public Instance wekaInstanceTrainingOrder(Dataset train, int ts,
		...
		for (int i = 0; i < wekaAtts.size(); i++) {
			...
			if (nameToAttribute.containsKey(attr.name())) {
				data.dataset.Attribute att = nameToAttribute.get(attr.name());
				if (att.hasContinuousValues()) {
					if (uD) {
						try {
	...
	public void selectAttributesUsingDiscretization(double[][] thresh, Dataset ref) {
		boolean[] vrs = new boolean[numAttributes()];
		for (int i = 0; i < vrs.length; i++) {
			try {
	...
	public void selectAttributes(boolean[] vars, Dataset ref) {
	...
	public static void main(String[] args) throws Exception {
		...
		Dataset d = ltcsv.loadData();		// Throws Exception
		for (int i = 0; i < 10; i++) {
