...
public class ErrorBased extends Discretizer {
	...
	private boolean errorMDL(double[][] m_counts) {
		double sumS1 = util.MathUtil.sumArray(m_counts[0]);
		double sumS2 = util.MathUtil.sumArray(m_counts[1]);
	...
	private double[] runError(double[] attVals, double[] classCounts,
		...
		if (examSameClass(initCount))
			...
			if (errorMDL(bestCount))
				...
				left = runError(util.Arrays.subArray(attVals, 0, bestI + 1),
						util.Arrays.subArray(classCounts, 0, bestI + 1),
						binsLeft);
				if (left != null)
					numBinsLeft = maxNumBins - left.length;
				right = runError(util.Arrays.subArray(attVals, bestI + 1,
