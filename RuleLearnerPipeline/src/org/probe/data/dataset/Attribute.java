...
public class Attribute implements Cloneable {
	...
	public Attribute(String name, DefaultDataModel refDataModel) {
		this(name, false, refDataModel);
	}

	public Attribute(String name, boolean isCont, DefaultDataModel refData) {
		this.name = name;
		isContinuous = isCont;
		wasContinuous = false;
		...
		isClassAtt = false;
		isIdAtt = false;
		refDataModel = refData;
		dblInd = -1;
		//la = null;
	}

	public Attribute(String name, int pos, double[] values, DefaultDataModel refData) {
		this.name = name;
		refDataModel = refData;
		position = pos;
		dblInd = position;
	...
	public Attribute(String name, int pos, String[] values, DefaultDataModel refData) {
		this.name = name;
		refDataModel = refData;
		position = pos;
		dblInd = -1;
	...
	public Attribute(String name, int pos, HashMap<String, Double> VTR,
			HashMap<Double, String> RTV, double[] vals, DefaultDataModel refData) {
		this.name = name;
		refDataModel = refData;
		position = pos;
		isContinuous = false;
	...
	public Object clone() {
		Attribute clone = new Attribute(this.name, refDataModel);
	...
	public VHierarchyNode hierarchy() throws AttributeDoesNotExistException {
		return refDataModel.getHierarchy(position());
	}
	...
	public void setReferenceDataModel(DefaultDataModel dataset) {
		refDataModel = dataset;
	}
		...
		if (nattToRep != null && valToRep != null) {
			...
			for (String v : nattToRep.keySet()) {
				if (!valToRep.containsKey(v)) {
					...
					numAdd++;
					try {
						refDataModel.getHierarchy(position()).addValue(
								new DiscreteNode(name(), v));
					} catch (AttributeDoesNotExistException e) {
						try {
							refDataModel.setHierarchy(position(), genHierarchy());
