...
public class D2S extends Discretizer {
	...
	private double[] runD2(double[] vals, double[] classIs, int[] classCounts) {
		...
		int[][] currSplit = org.probe.util.Arrays.init(
				new int[2][classCounts.length], 0);
		int[][] bestSplit = org.probe.util.Arrays.init(
				new int[2][classCounts.length], 0);
		...
		for (int i = 1; i < vals.length - 1; i++) {
			...
			double currEnt = org.probe.util.MathUtil.entropy(currSplit);
			if (currEnt < minEnt) {
				ind = i;
				minEnt = currEnt;
				bestSplit = currSplit;
		...
		if (leftSplit.length + 1 >= maxBins) {
			double[] nsplits = org.probe.util.Arrays.concatenate(bins, leftSplit);
		...
		bins = org.probe.util.Arrays.concatenate(bins, leftSplit);
		rightSplit = runD2(rightVals, rightCIs, bestSplit[1]);
		if (rightSplit.length + bins.length >= maxBins) {
			double[] nsplits = org.probe.util.Arrays.concatenate(bins, rightSplit);
			if (nsplits.length == maxBins)
				return nsplits;
			double[] msplits = new double[maxBins];
			System.arraycopy(nsplits, 0, msplits, 0, maxBins);
		...
		bins = org.probe.util.Arrays.concatenate(bins, rightSplit);
		return bins;
	}
