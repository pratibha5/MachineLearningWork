...
public class FayyadIraniEntropy extends Discretizer {
	...
	private double[] runMDL(double[] attVals, double[] classCounts, int binsLeft) {
		...
		if (examSameClass(initCount))
			...
			if (!FayaadIranniMDL(bestCount, initCount, numCutPts)
				...
				left = runMDL(org.probe.util.Arrays.subArray(attVals, 0, bestI + 1),
						org.probe.util.Arrays.subArray(classCounts, 0, bestI + 1),
						binsLeft);
				if (left != null)
					numBinsLeft = maxNumBins - left.length;
				right = runMDL(org.probe.util.Arrays.subArray(attVals, bestI + 1,
	...
	private double delta(double[][] m_counts, double[] initCount,
		...
		double LeftEntrop = org.probe.util.MathUtil.entropySet(m_counts[0]);
		double RightEntrop = org.probe.util.MathUtil.entropySet(m_counts[1]);

		return org.probe.util.MathUtil.log2(Math.pow(3, nTotalClass) - 2)
	...
	private boolean FayaadIranniMDL(double[][] m_counts, double[] initCount,
			int numCutPts) {
		double priorEnt = org.probe.util.MathUtil.entropySet(initCount);
		double currEnt = org.probe.util.MathUtil.entropySets(m_counts);
		double IGain = priorEnt - currEnt;

		return (IGain > (org.probe.util.MathUtil.log2(numCutPts) + delta(m_counts,
				initCount, priorEnt))
				/ (org.probe.util.MathUtil.sumArray(initCount)));
	}
